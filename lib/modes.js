/**
 * AccessibilityAlly modes — the operating mode the agent uses to summarize/explain.
 *
 * Four verb-named modes cover the matrix: translate (for a cross-functional PM),
 * brief (for a time-poor exec), onboard (for a no-tribal-knowledge new hire),
 * simplify (for a11y, ESL, neurodivergent, non-technical readers in one variant).
 */

/** @typedef {'translate' | 'brief' | 'onboard' | 'simplify'} ModeId */
 ...
 * @property {string} prompt - Mode-specific guidance appended to the agent system prompt.
 */
...
export const DEFAULT_MODE_ID = /** @type {ModeId} */ ('translate');
...
export function getMode(id) {
  return MODES.find((m) => m.id === id) ?? /** @type {Mode} */ (MODES.find((m) => m.id === DEFAULT_MODE_ID));
}

/**
 * Build the mode-specific suffix appended to the agent system prompt.
 * @param {string | undefined | null} id
 * @returns {string}
 */
export function modePromptFragment(id) {
  const mode = getMode(id);
  return [
    '',
    '## ACTIVE MODE',
    `The user has selected the *${mode.label}* mode. The MODE below is your operating mode for this response — it overrides any conflicting guidance about length, tone, or structure in the system prompt.`,
    '',
    mode.prompt,
    '',
